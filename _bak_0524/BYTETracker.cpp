...
std::vector<byte_track::BYTETracker::STrackPtr> byte_track::BYTETracker::update(const std::vector<Object>& objects)
{
...

    {
        ...
        const auto dists = calcIouDistanceOnly(remain_tracked_stracks, det_low_stracks);
...

    {
        ...
        // 미확정 트랙도 IoU 기반으로 매칭
        const auto dists = calcIouDistanceOnly(non_active_stracks, remain_det_stracks);
                                                     ...
                                                     std::vector<STrackPtr> &b_res) const
{
    ...
    const auto ious = calcIouDistanceOnly(a_stracks, b_stracks);
...
std::vector<std::vector<float>> byte_track::BYTETracker::calcIouDistanceOnly(const std::vector<STrackPtr> &a_tracks,
                                                                               const std::vector<STrackPtr> &b_tracks) const
{
    // --------------------------------------------------------
