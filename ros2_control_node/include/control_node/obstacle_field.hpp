...
//
//  동작 원리 (방향별 감속):
//   각 센서는 방향 d_i 와 거리 r_i 를 준다.
//   이동하려는 속도벡터 v=(vx,vy) 가 그 센서 방향과 같은 쪽을 향할 때,
//   거리가 가까울수록 그 방향 성분을 더 많이 깎는다.
//     - r >= slow_dist : 영향 없음(scale=1)
//     - stop_dist < r < slow_dist : 선형 감속 (1→0)
//     - r <= stop_dist : 그 방향 성분 완전 차단(scale=0)
//
