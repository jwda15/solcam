...
구독:
  /oak/rgb/image_raw (sensor_msgs/Image)  OAK RGB (recognizer=hagrid일 때)
...
발행:
  ...
  /phone_cmd      (std_msgs/String)       폰 카메라 제어 (자리만 — 폰 연동 추후)
  /system_cmd     (std_msgs/String)       전원 등 시스템 (자리만)
  /gesture_ui     (std_msgs/String, JSON) LCD UI·디버그용 상태 스냅샷

구조: 인식(recognizer) → 상태기계(menu) → 이 노드는 사건을 토픽으로 번역만.
처리 주기: IDLE은 idle_rate(저주기)로 like만 감시, MENU 중엔 menu_rate.
"""
import json
...
from .recognizer import HagridYoloRecognizer, MockRecognizer
...
class GestureNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("menu_rate", 15.0)           # Hz, 메뉴 중 주기
        self.declare_parameter("trigger_hold", 1.5)         # s, 따봉 유지
        self.declare_parameter("select_hold", 1.5)          # s, 선택 유지
        ...
        self.ui_flags = {"oak_view": False}   # 'ui' 액션이 토글하는 LCD 표시 상태

        self.mock_mode = str(gp("recognizer")) == "mock"
        if self.mock_mode:
            self.recognizer = MockRecognizer()
            self.create_subscription(String, "/gesture_mock", self._mock_cb, 10)
            # mock은 이미지가 없으므로 타이머로 상태기계를 구동
            self.create_timer(self.menu_period, self._mock_step)
            self.get_logger().info("recognizer=mock (/gesture_mock 으로 주입)")
        else:
    ...
    def _image_cb(self, msg: Image):
        t = self._now()
        # 주기 절감: IDLE은 idle_rate, MENU는 menu_rate로만 추론
    ...
    def _to_bgr(msg: Image):
        """cv_bridge 없이 Image → BGR ndarray (bgr8/rgb8만 지원)."""
    ...
    def _execute(self, action):
        ...
        if action.kind == "mode":
            ...
            m.delta = bool(action.payload["delta"])
