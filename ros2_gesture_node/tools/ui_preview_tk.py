...
# 키보드로 손동작 흉내 (방향 = 화살표, 자전 = Z/X, 리셋 = 2)
KEY2GEST = {"l": "like", "1": "one", "2": "two", "3": "three", "4": "four",
            "k": "dislike",
            "up": "p_up", "down": "p_down", "left": "p_left", "right": "p_right",
...
class Preview:
    ...
    def _dock(self, snap):
        ...
        # 방향 메뉴(Wheel/Lift)면 글리프 카드, 아니면(개수 메뉴) 숫자+라벨 카드
        directional = any(it["gesture"].startswith("p_") for it in items)
        cw, ch, gap = (92, 78, 10) if directional else (150, 64, 14)
        n = len(items)
        x0 = (W - (n*cw + (n-1)*gap)) // 2
        y = H - ch - 30
        ...
        for i, it in enumerate(items):
            ...
            if active and repeating:
                self._rrect(x, y, cw, ch, 12, fill=WHITE, outline=""); txtcol = INK
            else:
            ...
            if directional:
                ...
                c.create_text(x+cw//2, y+ch//2, text=f"{GNUM.get(it['gesture'],'')}  {label}",
                              fill=txtcol, font=("Segoe UI", 15, "bold"))
        if hold_g == "dislike":
            self._gauge(W//2, y-16, prog)
        path = " > ".join(snap.get("path", []))
        c.create_text(x0, y-18, text=path, anchor="w", fill=DIM, font=("Segoe UI", 12))
